...
export class PropertiesVolume {
  ...
  enableFor(server: Application): void {
    ...
    this.logger.info('2 setting pre.apiKey.primary' + (config.get('pre.primaryApiKey') as string).length);
    ...
    if (server.locals.ENV !== 'development') {
      ...
      this.logger.info('3 setting pre.apiKey.primary' + (config.get('pre.primaryApiKey') as string).length);
      ...
      this.logger.info('4 setting pre.apiKey.primary' + (config.get('pre.primaryApiKey') as string).length);
